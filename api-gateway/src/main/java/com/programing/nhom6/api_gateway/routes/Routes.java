package com.programing.nhom6.api_gateway.routes;
...
public class Routes {
    ...
    @Bean
    public RouterFunction<ServerResponse> productServiceTypeRoute() {
        return route("product_service")
                .route(RequestPredicates.path("/api/product/type/{type}"), HandlerFunctions.http("http://localhost:8080"))
                .filter(CircuitBreakerFilterFunctions.circuitBreaker("productServiceCircuitBreaker",
                        URI.create("forward:/fallbackRoute")))
                .build();
    }
